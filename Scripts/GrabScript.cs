...
public class GrabScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i =0; i < m_hands.Length; i++)
        {
            ...
            if (pullEnabled && IsControllerActive(m_hands[i].m_controller) && m_hands[i].m_controller.GetButtonUp(SixenseButtons.JOYSTICK))
            {
                RaycastHit hit;
    ...
    IEnumerator bringToPlayer(Transform pulledObject, Transform hand, int handIndex)
    {
        ...
        grabbedObjects[handIndex].transform.localPosition = new Vector3(hand.position.x, hand.position.y, hand.position.z - grabbedObjectOffset);
